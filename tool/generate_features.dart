...
final Map<String, Map<String, dynamic>> resourceConfig = {
  // Paladin
  'lay-on-hands': {
    'currentUses': 5, 'maxUses': 5, 'recoveryType': 'longRest', 'calculationFormula': 'level * 5',
    'forcedType': 'resource_pool'
  },
  'divine-sense': {
    'currentUses': 1, 'maxUses': 1, 'recoveryType': 'longRest', 'calculationFormula': '1 + cha_mod',
    'forcedType': 'resource_pool'
  },
  ...
  'channel-divinity-2-rest': {
    'currentUses': 2, 'maxUses': 2, 'recoveryType': 'shortRest',
    'forcedType': 'resource_pool'
  },
  'channel-divinity-3-rest': {
    'currentUses': 3, 'maxUses': 3, 'recoveryType': 'shortRest',
    'forcedType': 'resource_pool'
  },
  'cleansing-touch': {
    'forcedType': 'action',
  ...
  'indomitable-1-use': {
    'currentUses': 1, 'maxUses': 1, 'recoveryType': 'longRest',
    'forcedType': 'resource_pool' 
  },
  'indomitable-2-uses': {
    'currentUses': 2, 'maxUses': 2, 'recoveryType': 'longRest',
    'forcedType': 'resource_pool'
  },
  'indomitable-3-uses': {
    'currentUses': 3, 'maxUses': 3, 'recoveryType': 'longRest',
    'forcedType': 'resource_pool'
  },
  ...
  'rage': {
    'currentUses': 2, 'maxUses': 2, 'recoveryType': 'longRest',
    'calculationFormula': 'level < 3 ? 2 : (level < 6 ? 3 : (level < 12 ? 4 : (level < 17 ? 5 : (level < 20 ? 6 : 99))))',
    'forcedType': 'resource_pool'
  },

  // Bard
  'bardic-inspiration-d6': {
    'currentUses': 1, 'maxUses': 1, 'recoveryType': 'longRest', 'calculationFormula': 'cha_mod',
    'forcedType': 'resource_pool'
  },
  'bardic-inspiration-d8': {
    'currentUses': 1, 'maxUses': 1, 'recoveryType': 'shortRest', 'calculationFormula': 'cha_mod', // Font of Inspiration at lvl 5
    'forcedType': 'resource_pool'
  },
  'bardic-inspiration-d10': {
    'currentUses': 1, 'maxUses': 1, 'recoveryType': 'shortRest', 'calculationFormula': 'cha_mod',
    'forcedType': 'resource_pool'
  },
  ...
  // Monk
  'ki': {
    'currentUses': 1, 'maxUses': 1, 'recoveryType': 'shortRest', 'calculationFormula': 'level',
    'forcedType': 'resource_pool'
  },
  ...
  // Sorcerer
  'sorcery-points': {
    'currentUses': 2, 'maxUses': 2, 'recoveryType': 'longRest', 'calculationFormula': 'level',
    'forcedType': 'resource_pool'
  },
  'flexible-casting': {
    'forcedType': 'bonus_action',
...
void main() async {
  ...
  final featuresEnData = await _loadJson(p.join(srdPath, '5e-SRD-Features.json')) as List;

  // 3. Load Shadow File (Translations)
  print('🇷🇺 Loading Shadow File (Features Translations)...');
  final featuresRuData = await _loadJson(p.join(srdRuPath, '5e-SRD-Features_RU.json'), optional: true) as List?;
  ...
  for (var fEn in featuresEnData) {
    ...
    if (resourceConfig.containsKey(index)) {
      ...
      if (resourcePool!.containsKey('forcedType')) {
        ...
        if (type == 'action' || type == 'bonus_action' || type == 'reaction' || type == 'free') {
        ...
        type = (resourcePool!['recoveryType'] == 'shortRest' || resourcePool!['recoveryType'] == 'longRest') 
            ? 'resource_pool' 
            : 'action'; 
      }
    }
    ...
    if (index.startsWith('fighting-style-') || index.contains('-fighting-style-')) {
      type = 'passive';
      iconName = _getFightingStyleIcon(index);
      associatedClass = 'Optional'; 
    } else if (index.startsWith('channel-divinity') || lowerName.contains('channel divinity')) {
      ...
      iconName = 'filter_2';
        ...
        if (associatedClass == 'Barbarian') iconName = 'fitness_center';
        else if (associatedClass == 'Bard') iconName = 'music_note';
        else if (associatedClass == 'Cleric') iconName = 'health_and_safety';
        else if (associatedClass == 'Druid') iconName = 'nature';
        else if (associatedClass == 'Fighter') iconName = 'swords';
        else if (associatedClass == 'Monk') iconName = 'self_improvement';
        else if (associatedClass == 'Paladin') iconName = 'shield';
        else if (associatedClass == 'Ranger') iconName = 'gps_fixed';
        else if (associatedClass == 'Rogue') iconName = 'visibility_off';
        else if (associatedClass == 'Sorcerer') iconName = 'bolt';
        else if (associatedClass == 'Warlock') iconName = 'psychology';
    ...
    if (type == 'passive' && resourcePool == null) {
      if (lowerDesc.contains('as a bonus action') || lowerName.contains('bonus action')) {
          type = 'bonus_action';
          actionEconomy = 'bonus_action';
      } else if (lowerDesc.contains('as a reaction') || lowerName.contains('reaction') || lowerDesc.contains('use your reaction')) {
          type = 'reaction';
          actionEconomy = 'reaction';
      } else if (lowerDesc.contains('as an action') || lowerDesc.contains('use an action') || lowerDesc.contains('use your action')) {
          type = 'action';
          actionEconomy = 'action';
    ...
    // 2. Channel Divinity Catch-all
       ...
       if (index == 'channel-divinity' || index.contains('-rest')) {
           ...
           if (resourcePool == null) {
               resourcePool = {'currentUses': 1, 'maxUses': 1, 'recoveryType': 'shortRest'};
           }
       } else {
           // Consumers (actions)
           type = 'action';
    ...
    if (resourcePool != null && type == 'passive') {
        type = 'action'; // Default to action to ensure visibility in active trackers
    }
    ...
    if (index.startsWith('fighting-style-') || index.contains('-fighting-style-')) {
        ...
        cleanNameRu = cleanNameRu.replaceAll('Боевой Стиль: ', '').replaceAll('Боевой стиль: ', '');
    }

    // --- SAFETY NET: Force data for critical features ---
    if (index == 'flurry-of-blows' || index == 'patient-defense' || index == 'step-of-the-wind') {
       usageCostId = 'ki';
  ...
  final virtualFeatures = [
    {
      'id': 'rage-action', 'nameEn': 'Rage', 'nameRu': 'Ярость', 'type': 'bonus_action', 
      'usageCostId': 'rage', 'associatedClass': 'Barbarian', 'iconName': 'fitness_center', 'actionEconomy': 'bonus_action',
      'descEn': 'Enter a rage as a bonus action.', 'descRu': 'Впасть в ярость бонусным действием.'
    },
    {
      'id': 'bardic-inspiration-action', 'nameEn': 'Bardic Inspiration', 'nameRu': 'Бардовское Вдохновение', 'type': 'bonus_action', 
      'usageCostId': 'bardic-inspiration', 'associatedClass': 'Bard', 'iconName': 'music_note', 'actionEconomy': 'bonus_action',
      'descEn': 'Use a bonus action to give a creature a Bardic Inspiration die.', 'descRu': 'Использовать бонусное действие, чтобы дать существу кость Бардовского Вдохновения.'
    },
    {
      'id': 'wild-shape-action', 'nameEn': 'Wild Shape', 'nameRu': 'Дикий Облик', 'type': 'action', 
      'usageCostId': 'wild-shape', 'associatedClass': 'Druid', 'iconName': 'nature', 'actionEconomy': 'action',
      'descEn': 'Magically assume the shape of a beast that you have seen before.', 'descRu': 'Магическим образом принять облик зверя, которого вы видели ранее.'
    },
    {
      'id': 'second-wind-action', 'nameEn': 'Second Wind', 'nameRu': 'Второе Дыхание', 'type': 'bonus_action', 
      'usageCostId': 'second-wind', 'associatedClass': 'Fighter', 'iconName': 'healing', 'actionEconomy': 'bonus_action',
      'descEn': 'Regain hit points equal to 1d10 + your fighter level.', 'descRu': 'Восстановить хиты в количестве, равном 1d10 + ваш уровень воина.'
    },
    {
      'id': 'divine-sense-action', 'nameEn': 'Divine Sense', 'nameRu': 'Божественное Чувство', 'type': 'action', 
      'usageCostId': 'divine-sense', 'associatedClass': 'Paladin', 'iconName': 'auto_awesome', 'actionEconomy': 'action',
      'descEn': 'Detect celestial, fiend, or undead within 60 feet.', 'descRu': 'Обнаружить небожителей, исчадий или нежить в пределах 60 футов.'
    },
    {
      'id': 'lay-on-hands-action', 'nameEn': 'Lay on Hands', 'nameRu': 'Наложение Рук', 'type': 'action', 
      'usageCostId': 'lay-on-hands', 'associatedClass': 'Paladin', 'iconName': 'healing', 'actionEconomy': 'action',
      'usageInputMode': 'slider',
      'descEn': 'Touch a creature to restore hit points from your pool.', 'descRu': 'Коснуться существа, чтобы восстановить хиты из вашего пула.'
    },
  ];
  ...
  print('✅ Done! Generated ${outputList.length} features in ${stopwatch.elapsedMilliseconds}ms.');
}
...
Future<dynamic> _loadJson(String path, {bool optional = false}) async {
  ...
  try {
    ...
    if (optional) {
      print('   ⚠️ Warning: Failed to parse optional file ${p.basename(path)}: $e');
      return null;
    }
