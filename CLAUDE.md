...
**Проект портирован на GitHub с тремя ветками:**
- `main` - основная ветка (stable releases)
- `claude` - **РАБОЧАЯ ВЕТКА ДЛЯ CLAUDE CODE** ⚠️
- `gemini` - ветка для Gemini AI

**ПРАВИЛА РАБОТЫ:**
1. **ВСЕГДА работать ТОЛЬКО в ветке `claude`**
2. **НИКОГДА не переключаться на `main` или `gemini`**
3. **Перед началом работы проверять**: `git branch` (должно показывать `* claude`)
4. **Все коммиты делать ТОЛЬКО в `claude`**
5. **Не мержить в `main` без явного разрешения пользователя**
...
**Если случайно оказались не в claude:**
```bash
git checkout claude
```
...
- **Прогресс**: Начало проекта (Session 1)
...
**Следующий шаг**: Начало разработки в ветке claude
