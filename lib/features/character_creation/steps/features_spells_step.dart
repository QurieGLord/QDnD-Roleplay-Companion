...
class FeaturesSpellsStep extends StatelessWidget {
  ...
  String? _findFeatureIdForStyle(String style, String classId) {
    ...
    final classSpecificId = '$classId-fighting-style-$style'; // e.g. fighter-fighting-style-archery
    if (FeatureService.getFeatureById(classSpecificId) != null) return classSpecificId;
    ...
    if (style == 'great_weapon') {
        if (FeatureService.getFeatureById('fighting-style-great-weapon-fighting') != null) return 'fighting-style-great-weapon-fighting';
        if (FeatureService.getFeatureById('$classId-fighting-style-great-weapon-fighting') != null) return '$classId-fighting-style-great-weapon-fighting';
    }
    if (style == 'two_weapon') {
        if (FeatureService.getFeatureById('fighting-style-two-weapon-fighting') != null) return 'fighting-style-two-weapon-fighting';
        if (FeatureService.getFeatureById('$classId-fighting-style-two-weapon-fighting') != null) return '$classId-fighting-style-two-weapon-fighting';
    }
          ...
          child: Column(
            ...
            children: [
              ...
              if (passiveFeatures.isNotEmpty) ...[
                ...
                // Optional header for passives
                // Text(l10n.passiveFeatures, style: Theme.of(context).textTheme.titleMedium),
                ...passiveFeatures.map((feature) => _buildFeatureCard(context, feature, locale, l10n)),
              ],
  ...
  Widget _buildSubclassSelection(BuildContext context, CharacterCreationState state, AppLocalizations l10n, String locale) {
    return Card(
      elevation: 4,
  ...
  Widget _buildFeatureCard(BuildContext context, CharacterFeature feature, String locale, AppLocalizations l10n) {
    return Card(
  ...
  Widget _buildFightingStyleChoice(
    ...
    final styles = _fightingStyleOptions[classId] ?? _fightingStyleOptions['fighter']!;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
             Row(
              children: [
                Icon(Icons.sports_martial_arts, color: Theme.of(context).colorScheme.secondary),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    feature.getName(locale),
            ...
            const SizedBox(height: 16),
            Text(l10n.chooseFightingStyle, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: styles.map((style) {
                ...
                if (featureId == null) return const SizedBox.shrink(); // Skip if not found
                
                final styleFeature = FeatureService.getFeatureById(featureId);
                final name = styleFeature?.getName(locale) ?? style.toUpperCase();
                final isSelected = state.selectedFeatureOptions[feature.id] == featureId;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(feature.getDescription(locale)),
            const SizedBox(height: 16),
            Text(l10n.choose, style: const TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
  ...
  Widget _buildSpellSelection(BuildContext context, CharacterCreationState state, AppLocalizations l10n, String locale) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
