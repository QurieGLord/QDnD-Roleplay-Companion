...
class _RageControlWidgetState extends State<RageControlWidget> {
    ...
    if (value) {
      ...
      if (pool.currentUses > 0 || isUnlimited) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text(
                'No Rage charges left!'), // Fallback if key missing, or add to arb
  ...
  Widget build(BuildContext context) {
    ...
    final cardColor =
        _isRaging ? activeColor.withOpacity(0.05) : colorScheme.surface;
    final borderColor =
        _isRaging ? activeColor.withOpacity(0.5) : colorScheme.outlineVariant;
                        ...
                        Column(
                          children: [
                            IconButton(
                              ...
                              onPressed: (pool.currentUses >= maxRage)
                                  ? null
                                  : () => setState(() {
                                        pool.restore(1);
                                        widget.character.save();
                        ...
                        children: [
                          ...
                          if (_isRaging) ...[
                            ...
                            Icon(Icons.check_circle,
                                size: 10, color: colorScheme.onError),
                          ]
                        ],
                      ),
