...
void main() {
  test('Verify bilingual_test_compendium.xml parsing', () async {
    ...
    final wizard = result.classes.firstWhere((c) => c.name['en'] == 'Code Wizard');
    ...
    expect(wizard.skillProficiencies.from.any((s) => s.contains('---RU---')), false);
    ...
    final cacheSlots = level1Features.firstWhere((f) => f.nameEn == 'Cache Slots');
    ...
    final legacy = result.backgrounds.firstWhere((b) => b.name['en'] == 'Legacy Maintainer');
    ...
    expect(legacy.feature.getDescription('en'), contains('languages that no one speaks'));
    expect(legacy.feature.getDescription('ru'), contains('языки, на которых никто больше не говорит'));

    // --- Verify Items ---
    final keyboard = result.items.firstWhere((i) => i.nameEn == 'Keyboard of Warriors');
    expect(keyboard.nameRu, 'Клавиатура Воинов');
    expect(keyboard.type, ItemType.weapon);
    
    // --- Verify Spells ---
    final helloWorld = result.spells.firstWhere((s) => s.nameEn == 'Hello World');
